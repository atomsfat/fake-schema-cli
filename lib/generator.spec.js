...
describe('fake schema', () => {
  ...
  it('should generate overriding max items', () => {
    ...
    return expect(generate(path.resolve(__dirname, '../fixtures/N-timesRepeated.json')))
      .to.eventually.have.length(1);
  });

  it('should generate using default values', () => {
    const generate = generator.configure({ useDefaultValue: true }, 'en', (filename) => generator.readFile(filename));

    return expect(generate(path.resolve(__dirname, '../fixtures/Defaults.json')))
      .to.eventually.have.property('country-name', 'Spain');
  });
});
